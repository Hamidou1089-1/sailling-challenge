...
from utils.generate_training_scenarios import generate_interpolated_curriculum_params, ScenarioInterpolationCurriculum
...
class DQNTrainer:
    ...
    def compute_shaped_reward(self, obs, next_obs, raw_reward, done, step):
        """
    ...
    def collect_episode(self, max_steps=200):
        ...
        for step in range(max_steps):
            ...
            # Shaped reward
            shaped_reward = self.compute_shaped_reward(obs, next_obs, raw_reward, done or truncated, step)
    ...
    def train(self, num_episodes, eval_freq=100, save_freq=500, verbose=True):
        """
        Boucle d'entraînement avec curriculum d'interpolation de scénarios.
        ...
        # Stats par phase
        stage_stats = {
            'Static Training': {'rewards': [], 'successes': []},
            'Gentle Dynamics': {'rewards': [], 'successes': []},
            'Full Scenarios': {'rewards': [], 'successes': []},
            'Interpolation Training': {'rewards': [], 'successes': []},
            'Complex Mix': {'rewards': [], 'successes': []},
            'Full Spectrum': {'rewards': [], 'successes': []},
        }
        
        current_stage = None
        
        for episode in range(num_episodes):
            # Générer l'environnement avec interpolation
            init_params, evol_params, scenario_description = \
                generate_interpolated_curriculum_params(episode, num_episodes)
            
            
            curriculum = ScenarioInterpolationCurriculum(num_episodes)
            ...
            if stage_name != current_stage:
                if current_stage is not None:
                    # Logger les stats de la phase précédente
                    stats = stage_stats[current_stage]
                    if stats['rewards']:
                        avg_reward = np.mean(stats['rewards'])
                        success_rate = np.mean(stats['successes']) * 100
                        print(f"\n{'='*60}")
                        print(f"Stage '{current_stage}' completed:")
                        print(f"  Episodes: {len(stats['rewards'])}")
            ...
            self.env = SailingEnv(
                wind_init_params=init_params, 
                wind_evol_params=evol_params
            )
            
            # Collect episode
            episode_reward, success = self.collect_episode()
            
            
            
            # Stocker dans les stats
            stage_stats[stage_name]['rewards'].append(episode_reward)
            stage_stats[stage_name]['successes'].append(success)
            ...
            if self.writer:
                ...
                # Logger le type de scénario
                # On encode le stage comme un nombre pour visualisation
                stage_idx = list(stage_stats.keys()).index(stage_name)
                self.writer.add_scalar('Train/CurriculumStage', stage_idx, episode)
            
            # Console log
            if verbose and episode % 10 == 0:
                current_lr = self.scheduler.get_last_lr()[0]
                print(f"Ep {episode:5d}/{num_episodes} | "
                    f"Stage: {stage_name[:20]:20s} | "
            ...
            if episode % eval_freq == 0 and episode > 0:
                # Évaluer sur chaque scénario du prof
                eval_results = self.evaluate_all_scenarios(n_episodes_per_scenario=3)
                
                if self.writer:
                    for scenario_name, (avg_reward, success_rate) in eval_results.items():
                        self.writer.add_scalar(f'Eval/{scenario_name}/Reward', 
                                            avg_reward, episode)
                        self.writer.add_scalar(f'Eval/{scenario_name}/SuccessRate', 
                                            success_rate, episode)
                    
                    # Moyenne sur tous les scénarios
                    overall_reward = np.mean([r for r, _ in eval_results.values()])
                    overall_success = np.mean([s for _, s in eval_results.values()])
                    self.writer.add_scalar('Eval/OverallReward', overall_reward, episode)
                    self.writer.add_scalar('Eval/OverallSuccess', overall_success, episode)
                
                print(f"\n[EVAL] Episode {episode}")
                print("  Performance par scénario:")
                for scenario_name, (avg_reward, success_rate) in eval_results.items():
                    print(f"    {scenario_name:20s}: R={avg_reward:6.2f}, "
                        f"Success={success_rate:5.1%}")
                print(f"  Overall: R={overall_reward:.2f}, Success={overall_success:.1%}\n")
                
                # Save best model
                if overall_reward > best_eval_reward:
                    best_eval_reward = overall_reward
                    self.save_model('checkpoints/dqn/best_model.pth')
                    print(f"  ✓ New best model! (Reward: {overall_reward:.2f})\n")
        ...
        # Afficher résumé par phase
        print("\n" + "="*80)
        print("TRAINING SUMMARY BY CURRICULUM STAGE")
        print("="*80)
    ...
    def evaluate_all_scenarios(self, n_episodes_per_scenario=5, max_steps=200):
        """
        Évalue l'agent sur chacun des 5 scénarios du prof.
        
        Returns:
            Dict[scenario_name, (avg_reward, success_rate)]
        """
        
        results = {}
        
        for scenario_name in ['static_headwind', 'training_1', 'training_2', 
                            'training_3', 'simple_static']:
            scenario = WIND_SCENARIOS[scenario_name]
            
            total_reward = 0
            successes = 0
            
            for _ in range(n_episodes_per_scenario):
                # Créer environnement avec ce scénario
                env = SailingEnv(
            ...
            avg_reward = total_reward / n_episodes_per_scenario
            success_rate = successes / n_episodes_per_scenario
            results[scenario_name] = (avg_reward, success_rate)
